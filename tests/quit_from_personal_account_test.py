...
from locators import personal_account, registration, name, email, registration_button, password, email_log_in, password_to_your_account, go_button, quit_from_personal_account
...
# Ожидание, пока пользователь не будет перенаправлен в личный кабинет
wait.until(EC.element_to_be_clickable((By.XPATH, personal_account))).click()
...
wait.until(EC.element_to_be_clickable((By.XPATH, personal_account))).click()
time.sleep(5)
wait.until(EC.visibility_of_element_located((By.XPATH, quit_from_personal_account))).click()
time.sleep(10)
